...
contract TorchTokenWrapper {
    using SafeMath for uint256;
    IERC20 public torch;

    constructor(IERC20 _torchAddress) public {
        torch = IERC20(_torchAddress);
    }
    ...
    function stake(uint256 amount) public {
        ...
        torch.transferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public {
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        torch.transfer(msg.sender, amount);
    }
...
contract TorchBurnEvent is TorchTokenWrapper, Ownable {
    ERC1155Tradable public torch;

    mapping(address => uint256) public lastUpdateTime;
    mapping(address => uint256) public light;
    mapping(uint256 => uint256) public nfts;

    event NftAdded(uint256 nft, uint256 light);
    ...
    modifier updateReward(address account) {
        if (account != address(0)) {
            light[account] = earned(account);
            lastUpdateTime[account] = block.timestamp;
        }
        _;
    }

    constructor(ERC1155Tradable _torchNFT, IERC20 _torchAddress)
        public
        TorchTokenWrapper(_torchAddress)
    {
        torch = _torchNFT;
    }

    function addNft(uint256 nftId, uint256 amount) public onlyOwner {
        nfts[nftId] = amount;
        emit NftAdded(nftId, amount);
    }
    ...
    function mintNFT(uint256 nft) public updateReward(msg.sender) {
        require(nfts[nft] != 0, "NFT not found");
        require(
            light[msg.sender] >= nfts[nft],
            "Not enough light to mintNFT for nft"
        );
        require(
            torch.totalSupply(nft) < torch.maxSupply(nft),
            "Max nfts minted"
        );

        light[msg.sender] = light[msg.sender].sub(nfts[nft]);
        torch.mint(msg.sender, nft, 1, "");
        emit Minted(msg.sender, nfts[nft]);
    }

    function burnTORCH(uint256 amount) public {
        super.burn(amount);
        uint256 light = amount.div(6).div(1e18);

        light[msg.sender].add(light);
    }
}
